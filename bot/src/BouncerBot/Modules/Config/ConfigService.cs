...
public readonly record struct GuildConfig(
        LogChannelsSetting? LogChannelsSetting,
        IReadOnlyDictionary<AchievementRole, AchievementLogChannelOverride> AchievementLogChannels,
        IReadOnlyDictionary<Role, RoleSetting> RoleSettings,
        IReadOnlyDictionary<AchievementRole, AchievementMessage> AchievementMessages,
        VerifySetting? VerifySettings);
...
public class ConfigService(BouncerBotDbContext dbContext) : IConfigService
{
    ...
    {
        var logSetting = await dbContext.LogSettings.AsNoTracking().FirstOrDefaultAsync(ls => ls.GuildId == guildId);
        var achievementLogSetting = await dbContext.AchievementLogOverrides
            .AsNoTracking()
            .Where(alo => alo.GuildId == guildId)
            .ToDictionaryAsync(alo => alo.AchievementRole);
        var roleSetting = await dbContext.RoleSettings
            .AsNoTracking()
            .Where(rs => rs.GuildId == guildId)
            .ToDictionaryAsync(ls => ls.Role);
        var messageSetting = await dbContext.AchievementMessages
            .AsNoTracking()
            .Where(am => am.GuildId == guildId)
            .ToDictionaryAsync(ls => ls.AchievementRole);
        var verifySetting = await dbContext.VerifySettings.AsNoTracking().FirstOrDefaultAsync(ls => ls.GuildId == guildId);

        return new (logSetting, achievementLogSetting, roleSetting, messageSetting, verifySetting);
    }
}
